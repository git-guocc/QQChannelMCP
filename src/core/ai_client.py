...
你的判断:"""
    ...
    async def test_connection(self) -> bool:
        """测试API连接：以请求成功且JSON可解析为通过，不依赖固定文案"""
        try:
            response = await self.generate_text("ping")
...
class AIClientManager:
    """AI客户端管理器 - 支持多个AI提供商
    接受任意配置对象（Settings 或 QQChannelConfig），优先使用环境变量提供的密钥。
    """
    
    def __init__(self, config: Any, preferred_provider: AIProvider = AIProvider.GITHUB_MODELS):
        self.config = config
    ...
    def get_client_by_provider(self, provider: AIProvider) -> Union[GeminiClient, GitHubModelsClient, OpenRouterClient]:
        """根据提供商获取AI客户端"""
    ...
    async def get_default_client(self) -> Union[GeminiClient, GitHubModelsClient, OpenRouterClient]:
        """获取默认AI客户端"""
        return self.get_client_by_provider(self.preferred_provider)
    
    async def get_available_client(self) -> Union[GeminiClient, GitHubModelsClient, OpenRouterClient]:
        """获取第一个可用的AI客户端"""
    ...
    async def auto_configure_provider(self) -> AIProvider:
        ...
        # 优先级: OpenRouter > GitHub Models > Gemini
        provider_priority = [
